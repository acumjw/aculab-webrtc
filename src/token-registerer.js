...
export class TokenRegisterer {
    ...
    constructor(userAgent, options = {}) {
        ...
        this.options = Object.assign(Object.assign(Object.assign({}, TokenRegisterer.defaultOptions()), { registrar: defaultUserAgentRegistrar }), TokenRegisterer.stripUndefinedProperties(options));
        ...
        if (this.options.regId && !this.options.instanceId) {
            this.options.instanceId = TokenRegisterer.newUUID();
        }
    ...
    processChange() {
        if (this.change_pending) {
            ...
            if (this.user_token) {
                this.options.extraHeaders = ["Authorization: Bearer " + this.user_token];
                this.register();
            } else {
        ...
        switch (this._state) {
            ...
            case RegistererState.Unregistered:
                if (newState === RegistererState.Unregistered) {
                    notify = this.force_notify;
                } else if (newState !== RegistererState.Registered && newState !== RegistererState.Terminated) {
                    invalidTransition();
                }
                break;
